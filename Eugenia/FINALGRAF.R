...
df <- readRDS("./materiales/df_hombre_real.rds")
df$anio <- as.integer(format(df$tiempo, "%Y"))
df$mes  <- as.integer(format(df$tiempo, "%m"))
dic     <- df[df$mes == 12, ]   # snapshot de diciembre (stock de fin de año)
...
letra_labels <- c(
  "0"="0·Sin clsf","1"="A·Agric/Pesca","2"="B·Minería",
  "3"="C·Alim/Tex","4"="D·Manufactura","5"="E·Electr/Gas",
  "6"="F·Construcción","7"="G·Comercio","8"="H·Hoteles/Rest",
  "9"="I·Transporte","10"="J·Finanzas","11"="K·Inmobil./Serv",
  "12"="L·Adm.Púb.","13"="M·Educación","14"="N·Salud/Social"
)
...
r34_desc <- c(
  ...
  "9100"="Hospitales",
  "5211"="Supermercados",
  "7410"="Activ. jurídicas",
  "5521"="Restaurantes/bares",
  "7493"="Limpieza de edif.",
  "7499"="Otras activ. empres.",
  "5233"="Perfumería/farmacia",
  "5231"="Farmacias",
  "5512"="Hoteles",
  "6521"="Bancos comerciales",
  "1810"="Confec. de ropa",
  "7010"="Activ. inmobiliarias",
  "6420"="Telecomunicaciones",
  "7020"="Alquiler inmuebles",
  "5239"="Otras ventas minor.",
  "6610"="Seguros/AFJP",
  "8530"="Universidades",
  "9309"="Otras activ. sociales"
)
...
# Pregunta: ¿Cambió el perfil etario de las mujeres que se incorporan al
#           mercado formal entre el inicio y el fin del período?
# ==============================================================================

# Obtener primera observación de cada trabajadora
...
# ============================================================
# Escala Y basada en los hombres
# ============================================================

dfm$anio <- as.integer(format(dfm$tiempo, "%Y"))

dfm_ord    <- dfm[order(dfm$id_trabajador, dfm$tiempo), ]
first_idxm <- !duplicated(dfm_ord$id_trabajador)

entrada_h <- data.frame(
  edad = dfm_ord$edad[first_idxm],
  anio = dfm_ord$anio[first_idxm]
)

entrada_h <- entrada_h[
  !is.na(entrada_h$edad) &
    entrada_h$edad >= 17 &
    entrada_h$edad <= 65,
]

entrada_h$periodo <- NA
entrada_h$periodo[entrada_h$anio >= 1996 & entrada_h$anio <= 2000] <- "1996–2000"
entrada_h$periodo[entrada_h$anio >= 2017 & entrada_h$anio <= 2021] <- "2017–2021"
entrada_h <- entrada_h[!is.na(entrada_h$periodo), ]

# Obtener máximo de densidad de hombres
dens_h_96 <- density(entrada_h$edad[entrada_h$periodo == "1996–2000"])
dens_h_21 <- density(entrada_h$edad[entrada_h$periodo == "2017–2021"])
...
scale_y_continuous(limits = c(0, y_max_hombres))
...
medianas$x_label <- medianas$edad + c(-2, 2)
medianas$y_label <- c(0.065, 0.055)

paleta_p <- c("1996–2000" = "#4cc9f0", "2017–2021" = "#f72585")
...
p1 <- ggplot(entrada, aes(x = edad, fill = periodo, color = periodo)) +
  ...
  scale_fill_manual(
    values = paleta_p,
    labels = c(
      sprintf("1996–2000 (n = %s)", format(n_96, big.mark = ".")),
  ...
  scale_y_continuous(
    limits = c(0, y_max_hombres)
  ) +

  labs(
    title = "¿A qué edad ingresan las mujeres al empleo formal? — MLER",
    ...
    caption = "Fuente: MLER · Primera observación de cada trabajadora en la muestra"
  )
ggsave("g1_edad_entrada_periodos.png", p1, width = 13, height = 7, dpi = 150, bg = "#0a0a1a")

print(p1)
# ==============================================================================
# GRÁFICO 2 — Heatmap duración de relaciones laborales por LETRA
# Pregunta: ¿En qué sectores las mujeres tienen vínculos más cortos o más largos?
# ==============================================================================

# Duración = cantidad de meses registrados por relación
rel_key_all <- paste(df$id_trabajador, df$id_relacion)
dur_tab_all <- tapply(df$tiempo, rel_key_all, function(x) length(unique(x)))
let_tab_all <- tapply(df$letra,  rel_key_all, function(x) as.integer(names(which.max(table(x)))))

dur_letra_df <- data.frame(
  dur   = as.integer(dur_tab_all),
  letra = as.character(let_tab_all[names(dur_tab_all)])
)
dur_letra_df$dur_grp <- cut(dur_letra_df$dur,
...
tab_dur_letra$LETRA_LAB <- factor(tab_dur_letra$LETRA,
                                  levels = as.character(0:14), labels = letra_labels)
...
p2 <- ggplot(tab_dur_letra, aes(x = DUR, y = LETRA_LAB, fill = prop)) +
  ...
  labs(
    title    = "Duración de Relaciones Laborales por Sector (letra) — Mujeres (MLER 1996–2021)",
...
ggsave("g2_duracion_letra_heatmap.png", p2, width = 12, height = 8, dpi = 150, bg = "#0a0a1a")
print(p2)

# ==============================================================================
# GRÁFICO 3 — Stock de mujeres por grupo etario × año (área apilada)
# Pregunta: ¿Cómo cambió la composición etaria del empleo formal femenino?
# ==============================================================================

dic_ed  <- dic[!is.na(dic$edad), ]
dic_ed$grp <- cut(dic_ed$edad,
...
# ------------------------------------------------
# Hombres (para fijar la escala)
# ------------------------------------------------
dfm$anio <- as.integer(format(dfm$tiempo, "%Y"))
dfm$mes  <- as.integer(format(dfm$tiempo, "%m"))

dic_h <- dfm[dfm$mes == 12, ]

dic_h_ed <- dic_h[!is.na(dic_h$edad), ]

dic_h_ed$grp <- cut(
  dic_h_ed$edad,
  breaks = c(17, 25, 30, 35, 40, 45, 50, 55, 85),
  labels = c("17-24","25-29","30-34","35-39",
             "40-44","45-49","50-54","55+"),
  right = FALSE
)

tab_ea_h <- as.data.frame(table(GRP = dic_h_ed$grp, ANIO = dic_h_ed$anio))
tab_ea_h$ANIO <- as.integer(as.character(tab_ea_h$ANIO))

# ------------------------------------------------
# Escala común basada en hombres
# ------------------------------------------------
total_max_hombres <- max(
  aggregate(Freq ~ ANIO, tab_ea_h, sum)$Freq
)

total_max <- total_max_hombres
...
p3 <- ggplot(tab_ea, aes(x = ANIO, y = Freq, fill = GRP)) +
  ...
  geom_vline(xintercept = hitos$x, color = "black",
             linetype = "dashed", linewidth = 0.45, alpha = 0.6) +
  annotate("text", x = hitos$x + 0.1, y = total_max * 1.05,
           label = hitos$label, color = "black", size = 2.1,
           hjust = 0, vjust = 0, lineheight = 0.85) +
  ...
  scale_y_continuous(
    limits = c(0, total_max_hombres * 1.15),
    labels = comma_format(
      big.mark = ".",
      decimal.mark = ","
    ),
  ...
  labs(
    title    = "Evolución del Stock de Mujeres en Empleo Formal por Grupo Etario",
    subtitle = "Observaciones de diciembre de cada año · MLER 1996–2021",
    x        = NULL,
    y        = "Trabajadoras en stock (dic)",
    caption  = "Fuente: MLER · Elaboración propia"
  )
ggsave("g3_stock_edad_apilado.png", p3, width = 13, height = 7, dpi = 150, bg = "#0a0a1a")
print(p3)

# ==============================================================================
# GRÁFICO 4 — Heatmap: ¿qué edades concentra cada r34?
# Pregunta: ¿Existe segregación etaria entre ramas de actividad?
# ==============================================================================

df_r34_ed <- df[df$r34 %in% as.integer(top20_r34) &
                  !is.na(df$edad) & df$edad >= 17 & df$edad <= 70, ]

df_r34_ed$grp_edad <- cut(df_r34_ed$edad,
                          breaks = c(17, 22, 27, 32, 37, 42, 47, 52, 57, 70),
                          labels = c("17-21","22-26","27-31","32-36","37-41","42-46","47-51","52-56","57+"),
                          right  = FALSE)

# Calcular mediana de edad por r34 para ordenar las filas
med_edad_r34 <- tapply(df_r34_ed$edad, df_r34_ed$r34, median, na.rm = TRUE)
r34_ord_edad <- names(sort(med_edad_r34))   # de más joven a más mayor
...
tab_re$R34_LAB <- factor(tab_re$R34,
                         levels = r34_ord_edad,
...
p4 <- ggplot(tab_re, aes(x = EDAD_GRP, y = R34_LAB, fill = prop)) +
  ...
  labs(
    title    = "¿Qué edades concentra cada rama de actividad? — Mujeres (MLER 1996–2021)",
...
ggsave("g4_edad_por_r34_heatmap.png", p4, width = 13, height = 10, dpi = 150, bg = "#0a0a1a")
print(p4)
...
tab_r34_a$R34_LAB <- factor(tab_r34_a$R34,
                            levels  = top15_r34,
                            labels  = paste0(r34_desc[top15_r34], "  [", top15_r34, "]"))
...
p5 <- ggplot(tab_r34_a, aes(x = ANIO, y = R34_LAB, fill = idx_clamped)) +
  ...
  scale_fill_gradientn(
    colors = c("#8B0000","#C62828","#E53935","#1a1a2e","#1B5E20","#2E7D32","#66BB6A","#B9F6CA"),
    name   = "Índice\n(1996=100)",
    limits = c(20, 400)
  ) +
  scale_x_continuous(breaks = seq(1996, 2021, 3)) +
  # Marcar crisis 2002 y COVID 2020
  ...
  labs(
    title    = "Crecimiento del Stock de Mujeres por Rama de Actividad (1996–2021)",
    subtitle = "Índice base 1996 = 100 · Verde = expansión · Rojo = contracción · Obs. diciembre",
    x        = NULL,
    y        = NULL,
    caption  = "Fuente: MLER · Top 15 ramas r34 más frecuentes"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8))
ggsave("g5_crecimiento_r34_heatmap.png", p5, width = 15, height = 8, dpi = 150, bg = "#0a0a1a")
print(p5)
...
p5 <- ggplot(tab_r34_a, aes(x = ANIO, y = R34_LAB, fill = idx_clamped)) +
  ...
  scale_x_continuous(
    breaks = seq(1996, 2021, 3)
  ) +

  annotate(
    "rect",
    xmin = 2001.5, xmax = 2002.5,
    ymin = 0.5, ymax = 15.5,
    fill = NA,
    color = "yellow",
    linewidth = 0.9
...
# Pregunta: ¿Hay ramas con vínculos más precarios (cortos) que otras?
# ==============================================================================

df_r34_dur <- df[df$r34 %in% as.integer(top20_r34), ]
rel_key_r34 <- paste(df_r34_dur$id_trabajador, df_r34_dur$id_relacion)

dur_r34_tab <- tapply(df_r34_dur$tiempo, rel_key_r34, function(x) length(unique(x)))
...
dur_r34_df$dur_grp <- cut(dur_r34_df$dur,
                          breaks = c(0, 3, 6, 12, 24, 48, 300),
                          labels = c("1–3 m","4–6 m","7–12 m","13–24 m","25–48 m","48 m+"),
                          right  = TRUE)
...
r34_ord_dur <- names(sort(dur_med_r34))   # de menor a mayor duración mediana

tab_dr$R34_LAB <- factor(tab_dr$R34,
                         levels = r34_ord_dur,
...
p6 <- ggplot(tab_dr, aes(x = DUR, y = R34_LAB, fill = prop)) +
  ...
  labs(
    title    = "Duración de Relaciones Laborales por Rama (r34) — Mujeres (MLER 1996–2021)",
    subtitle = "Cada fila = 100% de relaciones de esa rama · Ordenado por duración mediana creciente ↑",
    x        = "Duración del vínculo laboral",
    y        = NULL,
    caption  = "Duración = meses con observación registrada · Fuente: MLER · Top 20 r34"
  ) +
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
ggsave("g6_duracion_r34_heatmap.png", p6, width = 13, height = 10, dpi = 150, bg = "#0a0a1a")
print(p6)
